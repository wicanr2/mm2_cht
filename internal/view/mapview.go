...
// 所以規則跟著紙本走：
//
// 不過原版也不是完全沒有俯視的東西：`.16` 的 `B` 系列（`TOWNB`／`CAVEB`／
// `CASTLEB`／`OUTB`）裡有藍底格子、四面牆的各種組合、門標，以及
// **↑ ↓ → ← 四個方向箭頭**，而且與地板／火炬／牆同一批載入
// （見 docs/formats/04-graphics.md）。那組圖磚顯然是給某種俯視顯示用的，
// 只是手上的截圖沒有一張拍到它，觸發條件未知 —— 所以這裡不去模仿它，
// 也不宣稱原版沒有。
//
//   - 五座城鎮：整張看得到（手冊本來就印了）
