...
//
// 武器的骰面數與加成在記錄的 +76…+79，由**已裝備的物品**算出來
// （`gear.go`，抄自 `sub_CE12`；物品表 +0x10 是骰面數）。
// 名冊裡沒裝備的人那幾格是 0，戰鬥層另有退路，見 `AttackDice`。
...
//
// 那個欄位由已裝備的物品算出來（`gear.go`），沒裝備時是 0。
// **沒有值時由力量推一個骰面數是 remake 自己加的**，原版沒有這條 ——
// 目的是讓赤手空拳的角色仍打得動，不是還原。
