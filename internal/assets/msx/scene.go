...
//
// **三支材質函式，不是「側牆一支正牆一支」**：`sub_2A57` 依地圖格的牆值
// 分派（1／2 → `sub_1103`、3 → `sub_1A40`、4 → `sub_1C2B`），每一支自己畫
// 左右側牆與正牆、四個深度，來源矩形是同一形狀平移 98 px。
//
// 這張表混了兩種：深度 0–2 的側牆取自材質 3，正牆與深度 3 的側牆取自
// 材質 1／2。**原版不會這樣混**，但三種材質沒有一種完整落在素材表內
// （材質 4 的右半與材質 1／2 的深度 2 都超出 462），而要照原版一格一格挑
// 材質得先解出地圖格的牆值位元。取捨與證據見
// `docs/research/02-other-platforms.md`，缺口是 `docs/todo.md` A8。
var scene = map[int]Piece{
	...
	// (434, 64) 56×21，右緣在 490，而素材表只有 462 寬。同一段的兩面側牆
	// （右緣 469 與 490）也一樣落在表外 —— 缺的是**一整塊來源**，
	// 不是某一筆座標抄錯：全遊戲最寬的圖就是 462，而 x ≥ 462 從來沒有
	// 任何一次載入寫過。在拿模擬器確認原版那裡畫的是什麼之前這一格不畫：
	// 寧可少一面牆，也不要拿越界或黑色的矩形頂替（`docs/todo.md` A8）。
