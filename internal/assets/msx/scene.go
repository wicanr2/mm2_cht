...
//
// **三支材質函式，不是「側牆一支正牆一支」**：`sub_2A57` 依地圖格的牆種
// 查該圖 `ATTRIB` 記錄的 +1／+2／+3，查到 1／2 交給 `sub_1103`、3 給
// `sub_1A40`、4 給 `sub_1C2B`，每一支自己畫左右側牆與正牆、四個深度。
//
// 這張表取的是**室內那組素材蓋得到的部分**：`sub_1A40` 讀的 y 0–61
// 給側牆，`sub_1103` 讀的 y 64–127 給正牆。`sub_1C2B` 與 `sub_1103` 的
// 深度 2 讀的是室內表沒有鋪到的座標（那幾條分支配的是戶外那組素材），
// 所以這裡沒有槽位。要照原版一格一格挑材質，缺的是牆種那個三位元欄位
// 怎麼取。證據見 `docs/research/02-other-platforms.md`，缺口是
// `docs/todo.md` A8。
var scene = map[int]Piece{
	...
	// VRAM (434, 320) 56×21，右緣 490 —— 那落在戶外那組的 `0x2042`／`0x2043`
	// （x 308–489）裡，室內這張 462 寬的表沒有鋪到。同一段的兩面側牆
	// 一樣。素材沒有缺，缺的是「室內會不會走到這條分支」的規則，
	// 在那之前這一格不畫：寧可少一面牆，也不要拿別組素材頂替。
	...
	//
	// 深度 1、2 的目的 x 同樣是算式（`42n + 63`、`14n + 70`），而 `n = 0`
	// 正好等於置中（`(154−28)/2 = 63`、`(154−14)/2 = 70`）—— 與牆面用的是
	// 同一條判準，所以這兩筆填得下來。**深度 3 沒有貼圖**（三支門常式裡
	// 沒有更小的來源），剩下的幾筆是橫向變體（`21n + 56` 這種），
	// 要先定出呼叫端的 `n` 才知道對應哪一格，在那之前不填。
