...
- 角色記錄：`0xFFCE9F` 起、每筆 **134 bytes**；`+00` 目前 HP、`+02` 上限
- 戰鬥驅動 `md-fight`：把仗打起來、`--invincible` 讓隊伍不會死、回合推得動

**還沒拿到的**：戰鬥結算的五個角色（`victory`／`enemy_killed`／
`member_killed`／`defeat`／`treasure`）。缺的是**把仗打完**，而卡點很具體：
原廠說明書講明八個戰鬥指令裡**只有 `Attack` 沒有後續提示**，但
「怎麼把選擇標記移到 `Attack`」三條路都不通 —— 不按方向鍵（標記跨回合保留，
停在 `Cast` 就一直進法術面板）、按住撞到頂（選單是環狀的，繞進 `Controls`）、
搜尋索引變數（等差搜尋的 8 個候選全是計時器）。
標記可能是指標或只存在 sprite 屬性表（VRAM，`m` 讀不到）。
