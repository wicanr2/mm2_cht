...
**還沒拿到的**：戰鬥相關的六個音樂角色要動態證據。走路已經不是問題，
缺的是**可靠的開打方式**：地圖 11 累計走約 160 步、連休 3 天，
隨機遭遇一次都沒觸發；固定遭遇格（`EVENTSO` 段 11 的 (2,3)／(1,4)／(2,4)／
(2,5)／(3,1)）走到了也沒打起來 —— 樣式是 `2b 01 12 …`，`2b` 是條件跳躍，
**「格子上有遭遇 opcode」不等於「踩上去會打」**。
還沒試：地城（Middlegate (8,0) 的 `0c 11 8f` 通到地圖 17）、競技場。
...
**還是開迴路。** 一步被吃掉或多走一步，後面整條路線都對不上而且看起來順暢。
要可靠得每步讀回隊伍 X／Y 重新對位 —— 那個 RAM 位址還沒定出來。
另外**起點差一格整條路線就會走進別的設施**：剛離開旅店是 (7,3)，
