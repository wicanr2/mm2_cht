...
**還沒拿到的**：戰鬥相關的六個音樂角色要動態證據。擋住的是**按鍵腳本走不完
野外的長路徑** —— 沒有一個按鍵在兩種狀態下都安全：`d`（C）有對話框時是「確認」、
沒有對話框時是「開啟指令選單」，而選單一開，後續方向鍵全變成選單導航。
實測 30 次 `Up;d` 的結果是隊伍一步沒走，最後停在 View Char 的改名框。
指令選單也不是按 `s` 關掉的，要選到最後一項 `Cancel`。

**下一步是讓按鍵腳本變成狀態驅動**：stub 本來就能讀記憶體，找出「目前有沒有
等待確認的訊息」那個旗標，每一步之前先讀它再決定送什麼鍵。

`/out/blastem-state` 目前存的是**出城後、Guardian Pegasus 事件已消耗、
人在野外**（地圖 11「Area C2」，(7,3) 往北）的狀態，可以直接接著用。
隨機遭遇不密：走約 40 格、連休 3 天都沒觸發。
