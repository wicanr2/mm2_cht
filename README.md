...
**調色盤用哪一段已經確定，顏色本身還沒。** 場景檔的 32 格裡
`0x16`–`0x1F` 是牆與地形，而 `0x03`–`0x11` 整段留黑 —— 那一段是給怪物的，
72 個 `.anm` 的索引用量正好集中在 `0x00`–`0x12`。只有六個 `.32` 會替那一段
上色，六個都畫過一遍，沒有一個像；總覽圖仍用 `book.32`，標為**假設**。
每個檔頂端 14 列是密度異常的非影像資料（用途未定），總覽圖裁掉。
細節與走不通的兩條路見
