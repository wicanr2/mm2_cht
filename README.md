...
基準是「距離一個可以公開發行的 1.0 還差什麼」。工作清單 35 項結案 30 項。
沒結的五項裡，三項卡在外部條件（建立公開儲存庫、Windows／macOS 真機驗收、
推廣片人耳驗收），另外兩項是裁決社群回報的原版 bug 時撈出來的 remake 缺口：
旅店休息只做了原版七件事裡的兩件半，以及沈睡與麻痺在 remake 不擋施法
（[`docs/todo.md`](docs/todo.md) G10 與 O10）。**九條原版 bug 已於 2026-08-20
逐條裁決完畢**（[`docs/research/07`](docs/research/07-blurglecruncheon.md)）。
...
（公開 repo、Windows／macOS 真機、推廣片人耳），加上裁決原版 bug 時撈出來的
兩項 remake 缺口（旅店休息的完整結算、沈睡與麻痺的擋人條件）。完整清單與
下一步在 [`docs/todo.md`](docs/todo.md)。
...
比對；496 條測試守著解出來的規格。所有工具鏈（56 支）都在 `tools/`。
