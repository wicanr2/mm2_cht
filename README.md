...
翻譯工作檔 2,702／2,702。這不代表每一條原版行為都取得了 parity oracle ——
...
|---|---|
| 中文化 | **2,702／2,702（100%）** |
| overlay 與記憶體佈局 | 14 個 overlay 全部反組譯，七個已無未解項；610 個函式符號有可查索引 |
...
| 戰鬥 | 九個指令已接上；近身的三十種附帶效果與怪物遠程／法術的三十二種都照原版分派；一般勝利留下戰利品，按 `S` 搜尋才進四選項寶箱；`F8` 一路打完 |
...
| Theme | DOS／Amiga／MSX2／Mega Drive／Modern 完整素材組可於啟動時選擇，遊戲中按 F6 原子切換 |
| 音樂 | **預設 Mega Drive**（16 首逐首從 ROM 擷取）；MSX／Amiga／DOS 音樂包可替換，公開 repo 不附原版音檔 |
...
還沒與原版對照過的有四項：DOS 水行術存檔生命週期、怪物施法的目標提示、
Mega Drive 戰鬥結算那五首的曲目對照、門狀態的事後複驗。**還缺的機制只有一項** ——
下攻擊指令時原版會列 `A`–`J` 讓玩家挑一隻，remake 的「打得到誰」已經照原版算，
但還沒有那個選單，所以固定打第一個站著的。未證實的項目不會用猜測阻塞正常遊玩，
完整清單與下一步在 [`docs/todo.md`](docs/todo.md)。
