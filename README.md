...
基準是「距離一個可以公開發行的 1.0 還差什麼」。工作清單 35 項結案 32 項，
**剩下三項全部卡在外部條件**：建立公開儲存庫、Windows／macOS 真機驗收、
推廣片人耳驗收。這台機器上做得完的都做完了。

社群回報的九條原版 bug 已於 2026-08-20 逐條回反組譯裁決完畢
（[`docs/research/07`](docs/research/07-blurglecruncheon.md)），裁決過程撈出的
三個 remake 缺口（裝備加成整套沒實作、旅店休息少做五件事、沈睡與麻痺不擋施法）
也一併補上。
...
| `F2` 的四個選項 | 事件獎賞自動入袋（預設開）、入夜視野變暗（預設關）、進設施先講一段場景（預設關）、離圖再回來的事件會不會重現（**預設會，照原版**）。理由與出處寫在 `docs/polish-spec.md` §4 |
| 原版的 bug | 社群回報的九條逐條回反組譯裁決完畢。**照抄為原則**，唯一做成開關的是「打倒的固定遭遇會重現」——那條同時是練功路線。見下面「原版的 bug：修了哪些、留了哪些」|
| 裝備加成 | 物品 `+0x0E` 的兩個 nibble（加哪一格、加多少）已接上：六個屬性、八種抗性、盜行與裝備防護值，加成量含該槽的附魔等級 |
| 旅店休息 | 照原版做七件事：清旅行效果、狀況只留詛咒／疾病／中毒、80 歲之後每次休息近半機率老死、中毒的人體力上限減半、食物扣一格（沒食物就不回復）、生病不回體力、法力上限重算。世紀不是現代時，每次休息有六分之一機率睡回現代 —— **那是回得來的唯一一條路** |
| 前端 | Ebiten 視窗；互動邏輯與視窗系統無關，headless 也跑得起來 |

玩家路徑上的機制都與原版對照過了。剩下的**只有要外部條件才做得了**的三項：
...
比對；508 條測試守著解出來的規格。所有工具鏈（56 支）都在 `tools/`。
...
| 設定 | 預設 | 另一邊 |
|---|---|---|
| 事件獎賞 | 自動入袋 | 照原版，按 `S` 才拿 |
| 入夜視野變暗 | 關（照原版，DOS 沒有日夜）| 開，借 Mega Drive 的做法 |
| 設施場景描述 | 關（照原版）| 開，顯示 Mega Drive 版的稿 |
| 離圖再回來的事件 | **會重現（照原版）** | 打過就不再出現 |

![設定](docs/screenshots/19-settings.png)

### 原版的 bug：修了哪些、留了哪些

1988 年的 DOS 版有一批社群記錄下來的 bug（來源是
[blurglecruncheon 的 MM2 站](http://www.geocities.ws/blurglecruncheon/mm2/)，
2001 年）。我們把九條逐條回反組譯查過，考證寫在
[`docs/research/07`](docs/research/07-blurglecruncheon.md)。

**先講結論：大部分「bug」在 DOS 版根本不存在，或成因與傳聞不同。**

| 傳聞 | 查下去的結果 | remake |
|---|---|---|
| 生命掉到負的會回捲到 65536 | **DOS 版沒這回事。** 生命、等級、屬性、經驗四條路的每一處減法前面都有守衛，扣不動就停在 0 | 同樣不會 |
| 中了沈默還是能在戰鬥外施法 | **真的。** 原版的擋人遮罩只收昏迷、麻痺、沈睡，沒有收沈默 | 照抄 |
| 穿透術往西會從 (0,0) 繞到 (15,15) | **落點是 (15,0)。** 兩個軸各自繞回，不是把地圖當一條線 | 照碼寫 |
| 附魔扣的法力差 512 倍，等級越高越便宜 | **方向反了。** 實際扣的是「50 × 目前附魔等級」——**等級 0 的東西升第一級不用錢**，之後越升越貴 | 照抄，等級 0 仍然免費 |
| 打倒的怪離開再回來又出現 | **真的**，而且不只沙拉金 —— 每一場固定遭遇都這樣 | **做成開關，預設不修** |
| 在水平面休息之後就游不動了 | **真的。** 休息會把整批旅行效果清掉，水行術在裡面 | 照抄 |
| 自然之門會把你從 800 年代送回現代 | **不是它。** 那條法術在別的世紀根本開不了門；把人送回現代的是**休息**，每次六分之一機率，落點是該地圖的預設入口 | 照抄 |
| 屬性被打到 0 之後脫下再穿上會憑空長回來 | **真的。** 加成加到上限 255、減到下限 0，兩邊夾法不對稱，所以卸下時扣不掉、穿上時照加 | **照抄，測試釘住** |
| 喝某口泉水會讓武士拿到法術選單 | 欄位查出來了（能不能施法在原版是**資料**決定的，不是職業），泉水寫哪一格還沒追 | 尚未重現 |

**唯一做成開關的是「打倒的怪會重現」那一條**，因為它同時是一條玩法 ——
原版的固定遭遇可以刷，當年的攻略就把它寫成練功路線。直接修掉等於替玩家
拿走一個選擇，所以**預設留在原版那一邊**；想要「打過就不再出現」的人到
`F2` 把第四項切過去，切了之後用掉的事件會被記住，而且進存檔。

其餘幾條一律**照原版**。這個專案的定位是保存，不是重新平衡 ——
知道它存在而決定照抄，與不知道它存在而漏掉，是兩件事。
...
這個 repo 的第一個 commit 是 2026 年 8 月 9 日，最後一個是 8 月 20 日。
十二天，460 個 commit。第一天就有 76 個 —— overlay 的載入機制、LZW 解壓、
`.16` 圖形格式、中文顯示、DOSBox oracle 全部在那一天打通。那天結束時，
一款 1988 年的遊戲已經能在自己寫的引擎裡跑出開場畫面，而且畫面上是中文。

後面的十一天沒有那麼順。

花掉最多時間的不是解不開的東西 —— 解不開的至少擺在那裡，知道自己還缺什麼。
貴的是**解錯了、而且自洽**的那些。`CONTEXT.md` 裡有一張表，記著 87 條被推翻的
斷言，每一條都寫了「當初憑什麼相信它、後來憑什麼推翻」。那張表比正確答案有用，
因為正確答案只在程式碼裡管用一次，而錯的方式會重複。

舉三個。第一個是牆的透空。先是「色號 8 一律透空」，錯了；改成「正牆不透空、
素牆看色號 8」，還是錯；再改成「門那一組拿素牆當遮罩取交集」，剩三個像素湊不出來。
三條規則接連「幾乎全對」，而錯得不明顯的規則比錯得明顯的難推翻得多。
最後去追貼圖到底怎麼搬的，一路追到 `EGA.DRV` 的跳表第 19 項，才發現透空
**根本與色號無關** —— 影像集裡本來就存著一張 1-bit 遮罩。畫師把遮掉的地方
填成色號 8，所以那三條重建出來的規則才會那麼像對的。

第二個是 `ITEMS.DAT` 的一個位元組。2026-08-15 那天下的結論是「程式只認 `0xF0`，
其餘 43 個相異值一次都沒被讀過，語意從二進位裡救不回來」。掃描是實際做過的 ——
十五個資料庫、十二處物品記錄的存取點，逐支讀完。錯的不是結論的推理，是**掃描的
樣式**：那次以「`0x6960` 加編號乘 20」當入口，而讀這一格的兩支把欄位偏移折進了
基底常數，寫成 `mov al, [bx+696Eh]`。同一個資料結構，換個寫法就掃不到。
那一格其實是裝備加成 —— 高四位選欄位、低四位給量，`Sage Robe` 加智慧、
`Fire Shield` 加抗火焰、`Skeleton Key` 加盜行，四十二個值逐件與名字對得上。
它被判死了五天，而 141 件有加成的裝備在那五天裡只剩名字。

第三個最短。8 月 20 日，三平台的包封好了，Docker 裡的 smoke 全過，
`go test ./...` 全綠，程式跑滿十二秒沒崩。實際打開包來玩，畫面**只剩空框** ——
框線在、地圖在、第一人稱視角在，中文一個字都沒有。原因是把畫布送進 Ebiten 之前
多呼叫了一次 `Flush()`，整層文字被原版像素蓋掉。離屏測試直接讀繪圖層的輸出，
結構上就看不到「送進視窗那一段」。從那天起，驗收多了一步：起 Xvfb、跑真的包、
截圖、看框裡有沒有字。

這些事加起來變成幾條寫在 `AGENTS.md` 與 `CLAUDE.md` 裡的規矩：掃不到先懷疑樣式，
不要先下「不存在」；規則在 N 個樣本上成立，不保證第 N+1 個；測試全綠只證明
內部自洽，不證明與原版一致；編譯成功不是視覺測試。

還有一類決定跟技術無關。社群記了九條原版 bug，我們逐條回反組譯查完 ——
有三條在 DOS 版根本不存在，有一條的方向和傳聞相反，剩下的是真的。
真的那些，一律照抄。附魔在等級 0 時不用錢、休息會把水行術關掉、屬性被打到 0
之後脫下再穿上會憑空長回來，這些都留著。只有一條做成開關：打倒的固定遭遇
離圖再回來又出現。留它的理由是它同時是一條玩法 —— 當年的攻略就把它寫成練功路線，
修掉等於替玩家拿走一個選擇。預設仍然是原版那一邊。

《魔法門 II》1988 年出，沒有官方中文版。三十八年後有了一個，
而且連它的 bug 一起留下來 —— 保存跟修好是兩回事，這個專案選的是前者。
