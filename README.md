...
基準是「距離一個可以公開發行的 1.0 還差什麼」。工作清單 36 項結案 31 項。
沒結的五項裡，三項卡在外部條件（建立公開儲存庫、Windows／macOS 真機驗收、
推廣片人耳驗收），另外兩項是讀完社群資料之後新開的：社群回報的九條原版 bug
還沒有一條回去對過反組譯，其中穿透術往西的座標包裹與我們讀到的
`sub_1C722` 對不上（見 [`docs/research/07`](docs/research/07-blurglecruncheon.md)）。
...
（公開 repo、Windows／macOS 真機、推廣片人耳），加上社群資料帶進來的兩項
待裁決（九條原版 bug、穿透術的座標包裹）。完整清單與下一步在
