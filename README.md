...
| 法術 | 96 條有 typed handler；隊員、物品、數字與飛行欄列提示已接正常 UI。**確認前不扣 SP／寶石**，與原版逐次量過一致；戰鬥中的怪物目標提示未證實，不猜補 |
...
還沒與原版對照過的有兩項：戰鬥中的怪物目標提示（要高等級的施法者才驗得到）
與 Mega Drive 戰鬥結算那五首的曲目對照。未證實的項目不會用猜測阻塞正常遊玩，
