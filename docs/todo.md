...
| A7 | Mega Drive 的火炬 remake 還沒接 | 實機截圖看得到牆上的火把，我們的沒有。原版走的是另一條路（`sub_34E8` ＋ `-$3742(a5)` 那張陣列），火焰顏色是逐幀換 CRAM 第 3 條的第 5–7 格、而且不吃光照 |
| A5 | Mega Drive 的光照沒接進 remake | 公式、級數、觸發條件都量完了（`research/02`「光照」），remake 目前一律用未調暗的原值。要接的話是在 `TownSet` 上加一個亮度係數（`分量 × 亮度 ÷ 8`，火把那三格例外不調暗）—— 但 DOS 版自己的暗處規則還沒查，兩邊要一起定 |
