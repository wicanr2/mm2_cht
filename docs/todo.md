...
| G6 | 非 DOS 平台的場景切換與野外視圖 | 場景素材依場景碼切換（P21）目前只有 **DOS 四套**（town／cave／castle ＋ 野外四個地形）與 **MSX 四張表**；Amiga、Mega Drive、高解析素材包都只有一套，地城與城堡沿用城鎮那組。**野外更缺**：`drawOutdoor` 只有 DOS 那組素材，其他平台在野外圖走回室內那條路徑 | 先查各平台的原版素材裡**有沒有**對應的場景組（Amiga 的 `.32`、Mega Drive 的場景區塊、MSX 的 `0x204x` 鑲嵌 —— MSX 那組已經知道是戶外用的，見 [`research/02`](research/02-other-platforms.md)）；有才談接，沒有就把「沿用城鎮」寫成刻意的取捨 | 按 `F6` 切到其他平台時，地城與野外看起來像城鎮。**DOS 那條是對的**，其他平台是退化 |
