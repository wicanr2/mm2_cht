...
| G4 | 野外兩側的「與前一格連續」分支 | 原版 `sub_18B0C`／`sub_18BEC` 在「這個深度也有正面擋路物」時改查**前一個深度**的影格與落點表，深度 1 的 x 另外強制 8（左）或 176／152（右）——那是把相鄰兩格的樹接成一片。remake 目前一律畫本深度那一筆，樹與樹之間會有縫 | 兩支的反組譯已經 dump 過（`workplace/ida/out/outdraw2.json`），照著補分支；驗收要**同一個座標同一個朝向**拿 DOSBox 與 remake 各畫一張比對，不是只看「像不像」 | 野外的樹在某些角度接不起來。**不影響玩，是外觀** |
