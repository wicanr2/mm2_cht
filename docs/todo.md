...
| A8 | MSX 沒定位的槽位 | **未解**：正牆深度 2 那一筆（`sub_1103` HL=2 的 else 支）要 (434, 64) 56×21，右緣在 490，而四張素材表都只有 462 寬；同一段的左側牆（SX=455、寬 35）右緣也是 490。**兩筆超出同一個量 ⇒ 缺的是那一段的來源基底，不是某一筆座標**。另外還有更深處的門與火炬（`sub_D10`／`sub_1956` 的更深分支）。下一步：從 VRAM 那頭反推 —— 載入位置只有 `(0,256)` 一處（`0x6854`，f002 行 8202），x ≥ 462 沒有任何一次載入寫過，所以要嘛還有一次載入沒找到，要嘛 `sub_1103` 有兩種基底。沒定位的格子**不畫**，見 [`research/02`](research/02-other-platforms.md) |
