...
| G3 | 野外的第一人稱素材 | **研究推進，尚未實作（2026-08-17）**。已經定出：野外是**另一條繪圖路徑**，`_2play_e10` 在場景碼 3／4／6 只載地板 `outf` 與**三組** `outdoor1-3`（`ds:036E`／`0372`／`0376`），不載「32 張牆」那組；畫的是 `sub_18AD0`（正面）／`sub_18B0C`（左）／`sub_18BEC`（右），每個深度各自挑素材組（`ds:54B0`／`54B4`／`54B8`，`0xFF` ＝ 沒有），左右還有「與前一格連續」的分支把樹接成一片；地平線橫帶由 `sub_189B8` 畫（素材 `ds:037A`，地形碼 `ds:5FD8`／`5FDC`／`5FE0`）。順帶把**室內**那條也釘死了（`_2play_e03`：正牆置中、側牆 8／32／64／88 ＝ remake 的 `sideX`，兩邊同一組數字）。實機對照拍好了（`workplace/dosbox/shots/42-outdoor.png`，米德格特北門出去是地圖 11）| **下一步（兩件事，做完才動手）**：① 讀 `sub_1BEBA`（617 bytes）——「這一格看到哪一種地形／擋路物」的填表規則；② 讀出 `ds:15A6` 起那八張落點表的值（做法同已讀出的那批：`EXE 偏移 = DGROUP + 0x8630`）| 24 張野外圖的第一人稱長得像城鎮。**能玩，但看起來不對** |
