...
| A8 | **推翻了自己的框架**：素材沒有缺 —— VRAM 是兩組素材輪流佔用同一片座標，戶外那組蓋住全部 72 筆；順帶解出 MSX 地圖檔＝DOS 的 `MAP.DAT` ＋ `ATTRIB` 記錄，並把火炬補到九個位置全滿、門補到三個深度全滿 | 牆種那個三位元欄位怎麼取（不需要模擬器）|
...
| A8 | MSX 沒定位的槽位 | **「素材缺一塊」是錯的框架（2026-08-17 推翻）**：貼圖參數是 **VRAM 絕對座標**，而 VRAM 上同一片座標由兩組素材輪流佔用 —— 室內載 `0x2020`–`0x2023`（462×128 蓋 x 0–461），戶外載一整片鑲嵌，其中 `0x2044`（196×62）蓋 **x 308–503**、`0x2042`／`0x2043`（182×102）蓋 **x 308–489 × y 320–421**。三個邊界與 `sub_1C2B`（右緣 503）、`sub_1103`（右緣 489、下緣 421）分毫不差，而逐筆對回載入表的結果是**戶外那組蓋住 72／72**（`tools/msxblits.py --vram`）。所以先前記的「x ≥ 462 沒有人寫過、要 openMSX 才分得出」**不成立**，那幾條分支配的是戶外素材。順帶解掉 MSX 地圖格式：**它就是 DOS 的檔案** ——`0x0100`–`0x0109` 每張 768 bytes ＝ 地形層（與 `MAP.DAT` 相同 255–256／256）＋ 屬性層 ＋ **該圖的 `ATTRIB` 記錄 64 bytes 逐位元相同**；材質就查那筆記錄的 +1／+2／+3。**剩一格**：`sub_2A57` 取牆種的三位元欄位（`rlca`×3 `and 7`）照 DOS 地形格式會混到東邊的位元，要嘛 `(688Fh)` 不是原始地形層、要嘛取法還有一步沒讀到 —— 解掉它才能斷言「室內走不到那幾條分支」，也才能照原版一格一格挑材質（remake 目前是照素材蓋得到的部分混用）。**火炬九個位置已經補齊**（左右深度 2、正面深度 1 與 2 —— 它們的目的 x 是算式`14n + 56`／`14n + 84`／`42n + 70`／`14n + 70`，掃立即值會抓到 `SY` 當目的 x）。**門的正面深度 1、2 也接上了**（`42n + 63`／`14n + 70`，`n = 0` 就是置中），深度 3 三支常式裡都沒有來源。剩下沒填的是橫向變體（`21n + 56` 這種），要先定出呼叫端的 `n`。見 [`research/02`](research/02-other-platforms.md)「同一張貼圖表，兩套素材」|
