...
| G3 | 野外的第一人稱素材 | 場景素材已經依場景碼切換（[`polish-spec`](polish-spec.md) P21），但**野外那三個場景碼（3／4／6，共 24 張圖）沒有對應的素材**，目前沿用城鎮那一套 —— 野外的牆不是 `out.16`（那個檔不存在），是地形圖 `DESERT`／`OCEAN`／`TUNDRA`／`SWAMP` ＋ `OUTDOOR1-3`，另一套機制 | 先解 `_2play_e10` 的 case 3／4／6 實際推哪幾個檔、地形怎麼決定用哪一張（`ds:16DA` 貼圖組編號已知，`0x0A` 是海洋），再決定 remake 怎麼畫 | 24 張野外圖的第一人稱長得像城鎮。**能玩，但看起來不對** |
