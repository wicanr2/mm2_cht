...
| A4 | Mega Drive 還沒歸類的 LZSS 區塊 | `tools/mdassets.py` 掃出 243 個只通過結構條件的區塊，其中 **10 個在 ROM 裡有人指到**，那十個才值得追（其餘多半是假陽性）。已知的兩個 1698 bytes 區塊（`0x06D05C`／`0x06F2E0`）長度不是 32 的倍數，形狀未定 |
