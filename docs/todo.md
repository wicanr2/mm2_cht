...
| G2 | 戰鬥中施法選怪物目標 | 原版問 `On which (A-J)?`，範圍是**場上全部**夾在 10（與近戰只問前排不同，O7 實機量到）。remake 的戰鬥施法目前不問目標，單體法術打第一隻站著的 —— 這是 fail-closed 的暫代規則，不是原版行為 | 照 `internal/ui/target.go` 的 `targetMenu` 再開一個給法術的版本（範圍不是 `Reachable(ranged)` 而是全場），接到 `SpellPromptFor` 上；效果那一層要收得下「打哪一隻」（目前單體法術沒有這個參數）| 單體攻擊法術等於自動鎖第一隻，玩家沒辦法集火後排 |
