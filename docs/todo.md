...
| G5 | 室內：兩側縱列的正牆 | `cmd/mm2diff -x 8 -y 0 -face E` 量到 **9.9%**，差異集中在走廊中距離的內緣。原版 `sub_1BEBA` 每個深度填三組：`我這一格 & 左方向遮罩` → 左側牆（影格 4–7）；**沒有的話**才 `左邊那一縱列的那一格 & 正面遮罩` → 那一列的正牆（影格 12／14／2／3，落點 x 8／8／40／88、y 22／40／54／62）；右側對稱（影格 13／15／2／3）。remake 目前只畫了深度 1 的補牆 | 照 `sub_1BEBA` 補齊三組的填法，再用 `cmd/mm2diff` 在**多個座標**量到 0.0%（現成的迴圈：印 timeline → host 跑 DOSBox → `-reuse` 比）| DOS ＋ 原版像素那條路徑的契約是「逐像素相同」，現在有些位置不成立 |
