...
| A7 | Mega Drive 的火炬 remake 還沒接 | **素材已定位**：ROM `0x06D05C` 的 53 個 tile，`sub_3836` 依視野格子值 3 直接動 nametable（不進組合緩衝區），火焰顏色是逐幀換 CRAM 第 3 條的第 5–7 格、不吃光照。**位置只量到深度 1**：8×24，view (37,45) 與 (163,45)。其餘深度要再取樣一次（找一張近距與遠距都有火炬的畫面），有了整組位置才接得進 `torchPlace` |
