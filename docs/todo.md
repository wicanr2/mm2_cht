...
| A8 | **推翻了自己的框架**：素材沒有缺 —— VRAM 是兩組素材輪流佔用同一片座標，戶外那組蓋住全部 72 筆；順帶解出 MSX 地圖檔＝DOS 的 `MAP.DAT` ＋ `ATTRIB` 記錄，並把火炬補到九個位置全滿 | 牆種那個三位元欄位怎麼取（不需要模擬器）＋ 門的深度 1–3 |
