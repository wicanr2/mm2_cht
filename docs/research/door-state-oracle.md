...
兩輪窄任務都**沒有取得一扇門成功後的 oracle**。正常玩家路徑可穩定重播到中門
第一人稱走廊；第二輪另抵達地圖 0 候選門 `(10,3,N)` 並由正常 `U` 指令操作，但只
得到既有事件／噴泉文字，沒有 `Success!`。因此「成功後立即前進、轉身、離圖再進、
休息、存檔重載」仍是 **unknown**，沒有把 remake 的門狀態寫成已證實。

已證實的既有資料仍可作前置證據：`2MISC.img` 的 `0xC130` 附近是門判定，撞門
handler 的成功跳轉在 `0xC19C`；`0xC2B2` 是開鎖流程。第二輪由既有
`workplace/ida/out/2MISC.asm` 進一步核對 `_2misc_e01`：`loc_1C289` 確認門值為 2，
`loc_1C326` 呼叫開門狀態處理後以訊息索引 4 顯示 `Success!`，失敗的
`loc_1C306` 進陷阱流程。這些仍是靜態定位，沒有成功前後 bytes，不能推出持久欄位。
...
5. 第二輪由乾淨 DOSBox 副本依 `Return → S → G → Z` 進入遊戲，再以逐鍵方向移動
   抵達地圖 0 候選門 `(10,3,N)`；按正常 `U` 指令後只顯示既有事件／噴泉文字，
   前後畫面一致。這是一個負結果，不是開鎖失敗或成功證據。
...
- **unknown：** 哪一面門牆能由乾淨起始隊伍正常觸發真正 `Success!`；候選
  `(10,3,N)` 已排除為目前路線下的有效成功 oracle。
