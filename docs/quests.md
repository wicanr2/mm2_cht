...
先前掛在這裡的五項已經解掉了，答案在 [`re/03-character-flags.md`](re/03-character-flags.md)
（`+125` 與 `+129` 的每一個位元、24 個全域夥伴旗標、三隻獸旗標的寫入時機）與
[`re/04-2brain-tavern.md`](re/04-2brain-tavern.md)（酒館，含說明書那句「到神殿解除任務」
在程式碼裡沒有對應）。剩下的是：

| 項目 | 卡在哪 |
|---|---|
| 酒館的點餐（`sub_1CD60`）與第五項（`sub_1D038`）做什麼 | 選單分派與價格已解，兩支的效果還沒讀完 |
| 酒館七筆訊息各自對到 `str.dat` 的哪一筆 | 訊息表 `ds:5722` 在執行時才填，落點在 `str.088` 那一段 |
