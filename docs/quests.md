...
目前沒有。先前掛在這裡的八項都解掉了 —— 角色記錄 `+125`／`+129` 的每一個位元、
24 個全域夥伴旗標、三隻獸旗標的寫入時機在
[`re/03-character-flags.md`](re/03-character-flags.md)；酒館的五個選單項、
餐點旗標、傳聞、以及訊息編號與 `str.dat` 索引的對應在
[`re/04-2brain-tavern.md`](re/04-2brain-tavern.md)；說明書那句「到神殿解除任務」
在程式碼裡沒有對應（取消任務的實作是 Uncle Spudly 的藥水）。

四十則傳聞的中英對照在 [`tavern-rumors.md`](tavern-rumors.md)。
