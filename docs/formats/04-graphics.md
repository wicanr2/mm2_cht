...
**未證實**：`sub_1CDA` 的第一個參數（怪物圖那次是 `0x16`）**怎麼對到
跳表的第幾項**。直接當索引去讀第 22 項（檔案偏移 `0x1233`），那裡是
「配置一塊記憶體再載入」的程序，看不出是 RLE 解碼 —— 可能對應關係不是
直接索引，也可能解碼在它呼叫的別處。`sub_1CDA` 開頭那二十幾條指令
完全沒碰第一個參數，它是先整理 `ds:0x4946` 那個結構才發出遠端呼叫的。

下一步：追 `ds:0x4A24` 這個 far pointer 是誰填的、填進去的偏移怎麼算，
才能把功能碼對到驅動裡的實際位址。
