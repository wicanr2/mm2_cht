...
2COMBAT.img 0xA3A1   call sub_7036(段號)
  ...
  → root 0x264A      載入該段，然後
                     sub_1CDA(0x16, 段的 far pointer, 緩衝 0x9E48,
                              輸出 0x9F78, 輸出 0x9F76)
  → sub_1CDA         顯示驅動的分派（讀 ds:4946 的呼叫表）
  → EGA.DRV 跳表第 22 項（0x16）→ 檔案偏移 0x1233
```

`EGA.DRV` 是 COM 檔，開頭是 **36 個 `jmp near`** 的跳表，功能碼就是索引。
`0x16` 那一項在檔案偏移 `0x1233`（COM 載入在 `0x100`，反組譯要 `--adjust-vma=0x100`）。

**所以 `MONSTERS.16` 的像素編碼要從 `EGA.DRV` 讀，不是從 `MM2.EXE` 或
overlay 讀。** 在主程式裡怎麼找都找不到，因為它根本不在那裡 ——
每種顯示卡一份驅動（`CGA`／`EGA`／`HGA`／`MCGA`／`TGA`），解碼與寫入
視訊記憶體的細節各自實作。

同一條路徑上的 `0x17` 是畫出來（`sub_2734` 呼叫），在 `0x1536`。
