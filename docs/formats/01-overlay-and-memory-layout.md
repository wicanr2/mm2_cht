...
十三筆的大小欄位 ×16 與實體檔案完全相符。**1MENU1 多 16 bytes**，
那 16 bytes 在**檔案最前面**，是重定位表：

```
0000  2B C7 00 00     重定位項 1：執行時偏移 0xC72B
0004  74 C6 00 00     重定位項 2：執行時偏移 0xC674
0008  00 × 8          補齊到一個 paragraph
0010  55 8B EC …      程式碼從這裡開始
```

項數就是描述表的 +0x02（1MENU1 = 2，其餘十三筆 = 0），每項 4 bytes，
補齊到 16 的倍數。兩個目標各自指向一條 `mov dx, imm16` 的 imm16
（file `0x554`、`0x60B` 都是 `BA 00 00`）—— 載入器把實際段值填進去。

證據是進入點：1MENU1 的兩個 thunk 目標換算成 overlay 內偏移是 `0x4A8`
與 `0xAAC`，兩處在檔案裡都是指令中間；**各加 16 之後都正好是 `55 8B EC`**
（`push bp; mov bp,sp`）。大小欄位因此就是讀取長度：跳過 16 bytes 的表，
往後讀 `0x00DA × 16 = 3,488` bytes，剛好到檔尾。
...
## 2.6 DGROUP 的初值段在 EXE 尾部

`ds:` 相對的位址落在 IDA 的 `seg019`，起點 linear `0x1D850`。
驗算：`ds:393h`（隊伍的 X 座標）→ `0x1DBE3`，正好落在 `byte_1DBE2` 旁。

IDA 把 `seg005`–`seg019` 整段宣告成 `db N dup(?)`，看起來像 BSS。
那是因為 **IDA 只載入 MZ header 宣告的 34,320 bytes**；實體檔案有 77,824，
尾部那 43,472 bytes 不在 image 裡，而它就是 DGROUP 的初值段。

```
EXE 檔內偏移 = DGROUP 偏移 + 0x8630
```

> 判斷某塊資料有沒有初值，看實體檔案，不看反組譯的段落屬性。
> 段落屬性反映的是載入器怎麼被告知，不是檔案裡有什麼。

### 這個位移怎麼驗的

拿執行時記憶體 dump 出來的九張表，每一張的位元組序列拿去整個 `MM2.EXE` 裡搜：

| DGROUP 偏移 | 內容 | EXE 檔內偏移 | 命中數 |
|---|---|---|---|
| `+0x0E3A` | 特殊攻擊的播報字串 | `0x0946A` | 1 |
| `+0x1012` | 每回合攻擊次數的職業除數 | `0x09642` | 1 |
| `+0x101A` | 同段程式碼的第二個除數 | `0x0964A` | 1 |
| `+0x10AA` | 特殊攻擊字串的指標表 | `0x096DA` | 1 |
| `+0x10EA` | 遭遇的難度門檻 | `0x0971A` | 1 |
| `+0x10F6` | 每類怪物的基礎編號與範圍 | `0x09726` | 1 |
| `+0x13F6` | 特殊攻擊旗標 A | `0x09A26` | 1 |
| `+0x1436` | 特殊攻擊的效果編號 | `0x09A66` | 1 |
| `+0x15E6` | opcode 長度表 | `0x09C16` | 1 |
| `+0x164E` | 城鎮設施的畫面換算表 | `0x09C7E` | 1 |

十張表、位移全部是 `0x8630`、每張在 77,824 bytes 裡都只命中一處。
再把 EXE 讀出的值與記憶體 dump 的值逐項比對，opcode 長度表 51 項全同。
兩個獨立來源一致。

### 尾部還有什麼

除了查表，這一段還有 836 條 NUL 結尾的可見字串：城鎮名、職業、種族、
陣營、次要技能、陷阱訊息、戰鬥播報、選單提示。中文化的主要戰場之一。
抽取在 `internal/assets/exetext`，翻譯走 `cmd/mm2strings` 的 `exe.XXXX` 這一組
（key 用 DGROUP 偏移，抽取規則調整了也不會整批位移）。
...
產出 `data/opcodes.json`、`combat.json`、`encounter.json`、`specials.json`，
引擎從那裡讀。這些是原版內容，不入版控。

## 2.8 從執行時的記憶體拿那些表（交叉驗證用）

`cmd/mm2data` 從 EXE 靜態取表之後，這條路不再是取得資料的必要手段，
但它是**第二個獨立來源**：偏移抓錯、欄位算錯，兩邊就對不起來。

```bash
tools/dosbox_run.sh ega "wait:3;key:Return;wait:2;key:s;wait:4;key:g;wait:5;key:z;wait:5;dump:ingame"
```

`dump` 步驟把 DOSBox 行程裡所有 1 MB 以上的匿名可寫記憶體倒出來
（需要 `--cap-add=SYS_PTRACE` 才讀得到 `/proc/<pid>/mem`）。
一定要**先走到第一人稱視角**再 dump —— 標題畫面時地圖與那些表都還沒填。

### 怎麼定出 DGROUP 在 dump 裡的位置

試過兩種不可靠的做法：

- 找 `MM2.EXE` 這個字串當錨點 —— dump 裡不只一處，算出來的 DGROUP
  讀到的是字型點陣。
- 找地圖資料 —— 讀檔緩衝裡也有一份，撞上的是緩衝不是 DGROUP。

**可靠的做法是拿表自己當 pattern**：用先前推出的 opcode 長度表的前十二個值
（`02 00 02 00 …`）去搜，全 117 MB 裡只中一處。DGROUP 就是
`命中位址 − 0x15E6`。這一招能用的前提是「推的表大致正確」，
而它正好也順便驗證了那張表。
...
| DGROUP 偏移 | 內容 |
|---|---|
| `+0x10EA` | 遭遇的難度門檻：`25, 40, 50, 55, 70, 75, 100` |
| `+0x10F6` | 每類怪物的基礎編號與範圍（四個一組） |
| `+0x1012` | 每回合攻擊次數的職業除數：`1,1,1,3,4,2,2,1` |
| `+0x101A` | 同段程式碼的第二個除數：`4,4,5,7,10,5,5,4` |
| `+0x15E6` | opcode 長度表（51 × uint16） |
| `+0x164C` 等五張 | 城鎮設施的畫面換算表 |

opcode 長度表拿原版的一比，先前推的 **50 個裡對了 46 個**。
錯的四個（`0x25`、`0x2a`、`0x2f`、`0x30`）在 1,363 段腳本裡出現得太少，
「每段必須恰好走到段尾」這條約束不足以把它們釘死。
...
|---|---|---|---|---|---|
| 1MENU2 | 2,846 | 25 | 2CAST1 | 1,623 | 49 |
| 2COMBAT | 5,569 | 117 | 2CAST2 | 1,801 | 64 |
| 2PLAY | 6,251 | 128 | 2CMDS | 1,792 | 37 |
| 1MENU1 | 1,405 | 27 | 2CAVES | 2,166 | 37 |
| 1RETINN | 1,115 | 12 | 2BRAIN | 2,006 | 24 |
| 2MISC | 1,543 | 32 | 2SMITH | 2,350 | 32 |
| 2MISC2 | 1,594 | 19 | 2TEMPLE | 1,498 | 21 |
