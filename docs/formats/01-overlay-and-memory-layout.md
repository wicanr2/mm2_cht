...
十三筆的大小欄位 ×16 與實體檔案完全相符。**1MENU1 少 16 bytes**。

那 16 bytes 是**程式碼加零填充**，不是重定位資料。檔案 `0xD90` 起的位元組流
是一段連貫的 DOS 收尾，橫跨 `0xDA0`（＝ `0x00DA × 16`）那條界線：

```asm
0D90  1E B4 25 CD 21     push ds; mov ah,25h; int 21h    ; 設中斷向量
0D95  58 5B 50 06 8E C3  pop ax; pop bx; push ax; push es; mov es,bx
0D9B  B4 49 CD 21 07     mov ah,49h; int 21h; pop es     ; 釋放記憶體
0DA0  58 1F 5F 5E 5D C3  pop ax; pop ds; pop di; pop si; pop bp; retn
0DA6  00 × 10                                            ; 對齊到段邊界
```

所以**大小欄位不是讀取長度** —— 少掉的那一段裡有 6 bytes 真的會執行。
載入器 `sub_17981` 的長度是另外兩個欄位相減算出來的
（`es:[di+0Ch] - es:[di+0Ah]`，再與 `es:[di+14h]` 取小）。

這同時抽掉了「+0x02 是重定位項數」的唯一佐證：那個推測靠的是
「1MENU1 是唯一非零的一筆，而它的尾端多出 16 bytes」，而尾端不是重定位資料。
**該欄位的語意退回未知。**
