...
意思是那些表**執行時才填**。追過三條路都不通：

1. **root 的程式碼**：`ds:15E6`、`ds:164C` 這些位址在 root 的反組譯裡
   連符號都沒有 —— 沒有 root 程式碼碰它們。
2. **載入程序**：`sub_160ED` 是通用的「開檔 → 查索引表 → 讀段 → 複製到
   DGROUP」，地圖、腳本、地圖屬性都走它，但沒有一個呼叫的目的地是那些表。
3. **EXE 的資料段**：`seg005`–`seg019` 全部宣告成 `db N dup(?)`，
   整個 DGROUP 區都是 BSS。檔案裡根本沒有那些表的初值。

反組譯 `1MENU1` 與 `2MISC` 兩個 overlay 也沒有找到寫入點。
