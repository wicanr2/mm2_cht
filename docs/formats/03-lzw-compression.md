...
```
+0  uint16  解壓後長度
+2  uint16  0          ← 恆為 0，是判斷「這裡是不是段頭」的特徵
+4  LZW 位元流
```

**第二個 word 不是 0 就不要當段解。** `SPELLS.DAT`（192 bytes 明文）的開頭
`00 01 80 01` 形狀很像段頭，硬解會得到「長度剛好 256」的垃圾 —— 解壓器讀完
位元流之後補零繼續，湊到宣告長度就停手。長度吻合在這裡是假陽性。
...
`sub_12242` 只在傳入的 `di != 0` 時對讀入的每個緩衝呼叫它。

### 這條路徑在出貨版本是關掉的

`word_1DD64` 落在 `seg019`（DGROUP 初值段，`base_para = 0x1D85`），
所以它就是 **`ds:0514`** —— 兩個名字指同一個 word。它的值是 `0x0316`，
而 DGROUP `0x0316` 那個位元組是 `00`：**金鑰是空字串**。

誰決定要不要解密，看 `sub_11E64`：

```asm
sub_11E64(檔名, 長度輸出):
    sub_11E36(檔名)                   ; 檔案在不在
    var_6 = sub_11ED2(0516h, 檔名)    ; ← 在「要解密」清單裡嗎 → 解密旗標
    if sub_11ED2(0518h, 檔名):        ; ← 在「走 LZW」清單裡嗎
        sub_121B6(檔名, 長度輸出, var_6)
    else:
        sub_124B0(檔名, 長度輸出, var_6)
```

兩張清單都是 NUL 結尾的字串指標陣列：

| 清單 | 內容 |
|---|---|
| `ds:0516`（要解密）| **空的** —— 第一個 word 就是 `0000` |
| `ds:0518`（走 LZW）| `map.dat`、`attrib.dat`、`monsters.dat`、`xxx` |

`sub_11E64` 的四個呼叫端都只推兩個參數（`add sp, 4`），**沒有人能從外面
把解密打開** —— 旗標一律來自那張空清單，所以恆為 0。

實測相符：`ds:0518` 那三個檔不做任何 XOR 就解得開
（`MAP.DAT` 第一段 512 bytes、`ATTRIB.DAT` 3,840 bytes、`MONSTERS.DAT` 6,656 bytes）。

**結論：XOR 解密的程式碼還在，但這一版沒有任何檔案走它。** 等級：**已證實**。
`sub_16818`（`monsters.16`）與 `sub_160ED`（`MAP.DAT` 的地圖記錄）另有自己的
載入路徑，兩支都寫死 `xor di, di`。

第四筆 `xxx` 不是遊戲檔案 —— 它是清單的哨兵，用來讓比對迴圈永遠有東西可比
（清單以 `0000` 結尾，`xxx` 只是最後一個不會命中的名字）。等級：**強推論**。
