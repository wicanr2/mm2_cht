...
| P23 | **野外的第一人稱**：三組擋路物 ＋ 地形帶 ＋ `OUTF` 地板，每一格畫什麼由地形值查表決定 | `_2play_e10` 野外分支只載 `outf` 與 `outdoor1-3`；`ds:52B2` 是地形值 → 碼；`sub_18AD0`／`18B0C`／`18BEC` 三支的影格與落點表；地形帶走 `sub_189B8`（[`04-graphics`](formats/04-graphics.md)）| `internal/view/outdoor_test.go` 兩條（含**拿真的素材尺寸驗每一張都落在視圖裡**）＋ 截圖 `01l-first-person-outdoor.png` 與實機 `42-outdoor.png` 對照 |
