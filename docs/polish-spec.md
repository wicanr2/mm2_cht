...
| P26 | **MSX 的戶外第一人稱**：一整排格子（每個深度列舉 ±5／±2／±1／±1 個橫向偏移），每格由碼挑三組擋路物之一 | `sub_2B0A` 列舉、`sub_2A57` 取格子位元組最高 3 bit 查每張地圖的表（`+0x201`）分派到 `sub_1103`／`sub_1A40`／`sub_1C2B`；落點由 `tools/msxout.py` 符號執行產生（[`research/02`](research/02-other-platforms.md)）| 截圖 `01r-outdoor-msx.png` ＋ `internal/assets/msx/outdoor_test.go` 兩條（產生出來的每一塊都要落在視圖裡、來源要落在素材表裡）|
