...
| P28 | **MSX 戶外的素材選擇與格子碼**：擋路物 A 與地形帶各有兩張、地面四張，換圖時依地圖號挑；地平線帶的變體有兩條判準；格子碼改讀 MSX 自己的地圖檔而不是 DOS 的分類 | f002 `0x431` 的 24 筆記錄（`sub_4F9`／`sub_37B0`）、`sub_390E` 的四格 `cp`、`sub_297A` 的兩條判準（[`research/02`](research/02-other-platforms.md)）| `internal/assets/msx/outdoor_test.go` 三條 ＋ 截圖 `01s-msx-tundra.png`／`01t-msx-desert.png` |
| P31 | **進設施先講一段場景**（`F2` 開，**預設關**）：五種設施 × 五座城共 25 段。**那是 Mega Drive 版的稿**，DOS 只有選單 | 區塊版面是 `uint8 長度 + 內容`；段對城的依據是訓練所那一塊在五段描述之後緊接著五個 DOS 設施名（[`research/03`](research/03-megadrive-text.md)）| `internal/ui/mdflavor_test.go` 三條 ＋ 截圖 `20-md-flavor.png` |
| P30 | **時間會走**：256 步一天、180 天一年、年份上限 999，跨日時每個隊員的年齡加一天（滿 181 天換一歲）。附帶一個 `F2` 選項「入夜視野變暗」，**預設關** | root `0x150CE` 與 `sub_15092`（[`formats/11`](formats/11-time-and-light.md)）。**變暗那一半不是 DOS 的**：時鐘是它的、繪圖路徑卻一處都沒讀，做法借自 Mega Drive（`分量 × 亮度 ÷ 8`，晝 8 夜 6），「一天的後半算夜晚」是 remake 挑的 | `internal/game/time_test.go` 兩條 ＋ 截圖 `01v-night.png`、`19-settings.png` |
| P29 | **沒有照明就看不見**：照明計數為 0 且這張圖沒有光源（`ATTRIB +26` bit 7）或這一格吃照明（屬性層 bit 5）時，第一人稱整塊不畫，視圖中間印一行字。走上吃照明的格子燒一點照明。順帶修掉開局漏清旅行效果（照明／魔法／力場／飄浮全掛著 1）| root `sub_13FFC`（`0x1401C` 起）與 `0x150CE`；字串 `ds:4DFB`（掃描器原本吃掉它，補進 `exetext.knownExtra`）| `internal/game/dark_test.go` 兩條 ＋ 截圖 `01u-darkness.png` |
