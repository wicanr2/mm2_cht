...
| `20-md-flavor.png` | 進設施先講一段場景（`F2` 開，**預設關**）。那是 Mega Drive 版的稿 —— DOS 走進神殿只有選單，沒有這一句 |
| `01v-night.png` | 入夜視野變暗（`F2` 開，**預設關**）。只暗視圖那一塊，隊伍面板與狀態列不動。DOS 沒有日夜 —— 時鐘是它的、變暗是借 Mega Drive 的 |
| `01u-darkness.png` | 沒有照明進地城：原版**整塊不畫第一人稱**，只在視圖中間印一行字（`ds:4DFB` ＝ `Darkness`）。它不是調暗，是不畫 |
| `01s-msx-tundra.png` | 換得掉的三個槽之一：地圖 5 載的是另一張擋路物（`0x2043`）與地形帶的第二段，所以是冰原不是樹林 |
| `01t-msx-desert.png` | 地圖 41–44 是四個元素位面（風火土水），格子在原版**什麼都不畫**（分派值落在 `sub_2A57` 認得的 1–4 之外，而碼 1–3 那條路在地圖 41 以後被閘門擋掉），看到的是換過地面的背景。用 DOS 的格子碼會多畫一層綠色的帶 |
