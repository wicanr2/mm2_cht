...
視圖區頂端那一片不是素材，是程式畫的。量自 `shots/fpv.png` 的 y 8–21
（208 px 寬）：黑與藍各 1,456 個像素、剛好一半一半，排成**橫向兩格一組、
逐列交錯**的棋盤：

	y 偶數  BB..BB..BB..
	y 奇數  ..BB..BB..BB

規則是 `((x−FPX)/2 + (y−FPY)) 為偶數就塗藍`（EGA 的 0 與 1）。
先鋪滿整個視圖區，地板與牆再蓋上去 —— 沒被蓋到的地方就是天花板，
深度不同露出來的高度自然就不同（`22-fpv2.png` 只露最上面幾列）。

城堡與地城用的是另一組貼圖（`CASTLE*.16`／`CAVE*.16`），花紋一不一樣
還沒對過截圖。
