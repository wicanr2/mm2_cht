...
|---|---|---|---|
| 第一人稱視圖 | 5..130 | 5..216 | |
| 狀態框 | 5..130 | 222..313 | 原版放 `Protection`：照明／魔法／力場。**三行的資料來源還沒解** |
| 橫列 | 133..146 | 5..313 | `'O' Options`、`Day=`、`Year=`、`Face=`（分隔線在 x 98–100、170–172、250–252）|
| 隊伍名單 | 149..186 | 5..313 | 兩欄各三列，每列「編號 名字 生命」，**沒有職業** |

**下方那一塊大框是兩用的**：`shots/fpv.png` 放隊伍名單，
`shots/c2.png`（神殿門口）放的是三行對話。所以訊息不必另外找地方擺 ——
有訊息就顯示訊息，沒有才顯示名單。
