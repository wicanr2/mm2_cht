...
| **通行規則** | 14 個原版實測樣本入庫；四種統計模型最高只到 12/14。正解在 `sub_15E68`：`and 55h` 證實 bit0/2/4/6 是通行位元、`shr`+`and 3` 證實是 2-bit 欄位、判定用兩個值的 AND。還要追 `ds:59C6/59C8/59CA` 怎麼填、`ds:59C7` 的移位量怎麼由方向決定。見 [`docs/formats/06`](docs/formats/06-map.md) §4 |
| **第一人稱視角** | 素材已備齊（`TOWN.16` 的 160×92 / 96×56 / 48×28 是三種深度的牆、`TOWNF.16` 地面、`TOWNT.16` 火炬），等通行規則解出來 |
