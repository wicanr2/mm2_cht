...
| `MONSTERS.16` RLE | 段內索引、動畫序列表、影像頭 x/y/w/h 已解，像素編碼未解 —— **但已定位到解碼器在 `EGA.DRV` 的功能 0x16（檔案偏移 0x1233），不在 MM2.EXE 或 overlay 裡**，六種用猜的讀法都已排除（見 docs/formats/04）。**哪隻怪物用哪一段已解**：怪物記錄 +0x15 |
