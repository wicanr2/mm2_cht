...
| `MAP.DAT` 其餘位元 | **地形層的 `bit 6/4/2/0` 分辨實牆與門**：把室內外分開量之後對向一致率 **99.7%**（先前的 91.7% 是把野外二十四張混進來的）。室內 36 張裡門佔牆面 1.8%（283 面）。**室外走另一套模型**（`sub_5F40` 把碼 `& 0x1F` 查 `ds:52B2` 的 32 項表分成 5 類），還沒實作。原版的 `sub_5E68` 證實了「每方向 2 位元、3=`Solid!`、2=`Locked!`」與牆位元 (6,4,2,0)（`and al, 55h`），但**那兩個位元各自取自哪個平面還沒定** —— 同位元組的讀法算出 92.8% 的牆是門，荒謬。屬性層的高位元平面（bit 7/5/3/1）仍未定 |
