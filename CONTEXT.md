...
| **法術表** | 96 條全備（牧師 48 + 巫師 48），名稱、消耗、形式、對象、說明一條不缺，`internal/game/spells.go`。資料抄自手冊，**不是從程式碼解出來的** —— 現在的用途是法術清單與施法介面的文字來源，不是戰鬥結算的依據 |
...
| **成長與設施** | `internal/game/facility.go`。升級（每級生命點數照手冊的職業表：武士／野蠻人 1–12、遊俠／弓箭手 1–10、牧師／賊／忍者 1–8、巫師 1–6）、法力等級對照（經驗等級 1/3/5/7/9/11/13/15/17）、旅店休息、神殿治療、訓練所升級、戰後給經驗。**升級所需經驗與神殿價目是暫定的** —— 手冊沒給，原版的表在 BSS |
| 經驗值欄位 | **記錄裡的位置未定**，所以 `Character.Exp` 只活在記憶體裡，存檔寫不回去。掃過名冊每一個 uint16／uint32，與等級高度相關的都是跨欄位的假相關 |
| **遊戲迴圈** | `internal/game/session.go` + `cmd/mm2play`。走地圖、觸發事件、依機率遭遇、開打、全滅判定、存檔。遭遇的怪物依所在地圖的編號取樣（**不是原版的遭遇表** —— 那張難度門檻表在 BSS） |
...
| **戰鬥數值是暫定的** | **命中與傷害的係數不是原版公式**。原版的判定用的查表在 DGROUP 的 BSS 區，執行時才填、檔案裡讀不到。現在照手冊寫明的規則做（力量影響傷害、準確度影響命中、速度增加防禦），擲骰走原版那顆 RNG。怪物數值由**難度序**推算而非那 12 個未解的位元組 —— 第一版拿 `+0x0F` 當等級，四百次攻擊命中零次。公式解出來之後換掉 `Resolve` 與 `NewMonster` 兩支函式即可 |
| 角色記錄的 SP | `+88`/`+90`（uint16 目前／上限）。只有牧師與巫師非零，與手冊「遊俠與弓箭手要高經驗等級才有法力」一致 |
| 角色記錄的其餘欄位 | 已定位 15 個：名稱、性別、陣營、種族、職業、六項屬性、等級、年齡、食物、狀況、HP、SP，以及屬性的第二份。狀況（+38）是位元遮罩，bit6 由 `sub_1AFBC` 在 HP 歸零時設、bit7 更嚴重。手冊列的人物資料畫面有 21 欄，還缺 AC、SL、經驗、黃金、寶石、盜行、第二技能、裝備、背包。運氣（第七項屬性）的位置也未定 —— 屬性區與其第二份都只有六項 |
