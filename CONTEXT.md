...
| **事件腳本的 opcode** | 50 個 handler 位址與長度已全部列出，語意解出九個：三個顯示字串（`01`/`02`/`04`）、兩對條件跳躍（`2b` 與 `10`/`11`）、兩個固定遭遇（`12`/`13`）、事件消耗（`14`）、重畫（`0f`）。按出現次數算，最常見的前六個已經解掉五個。見 [`docs/formats/07`](docs/formats/07-event-script.md) |
