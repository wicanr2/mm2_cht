...
角色創建的擲骰程序（`sub_189F8`）對 **7 格**清 0 並各填 3d7，
而 `party.go` 目前把 `+16` 起算成六格、`+22` 是抗性、運氣在 `+115`。
兩邊不能同時成立。

判定條件：拿到呼叫端傳進 `sub_189F8` 的指標，確認那塊是不是 `記錄+16`。
在那之前**不要改欄位表** —— 現行版本有名冊四十筆的量級比對撐著，
而擲骰程序只證明「傳進來的那塊是七格」。

細節見 `docs/formats/10-character-creation.md`。
