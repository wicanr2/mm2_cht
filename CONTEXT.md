...
| **戰鬥判定已換成原版的** | `sub_8398` 逐行讀出：命中率 = 命中門檻[難度層] − 目標防護等級（保底 5%），每次揮擊各擲 `rand(10,1009)/10` 判定，命中的傷害是 `rand(1, 骰面數)`。門檻表 `ds:103A` 與怪物的生命／經驗／攻擊次數／傷害骰全部到位。**仍是假設的**：隊伍打怪物的計算端還沒找到（訊息端已定位），怪物的防護與速度欄位還沒指認。見 [`docs/formats/08`](docs/formats/08-combat.md) |
