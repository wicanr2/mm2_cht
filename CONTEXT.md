...
| **原版的查表已取得** | DGROUP 整段是 BSS，檔案裡沒有初值 —— 改從執行時的記憶體拿：DOSBox 走到第一人稱視角後 dump 行程記憶體，用先前推出的 opcode 長度表當 pattern 定出 DGROUP。已取得遭遇門檻表、怪物編號表、職業攻擊次數除數、opcode 長度表、五張設施換算表。見 [`docs/formats/01`](docs/formats/01-overlay-and-memory-layout.md) §2.8 |
