...
| 角色記錄的其餘欄位 | 已定位 16 個（含 `+36` 防護等級，由 `sub_8398` 確認）：名稱、性別、陣營、種族、職業、六項屬性、等級、年齡、食物、狀況、HP、SP，以及屬性的第二份。狀況（+38）是位元遮罩，bit6 由 `sub_1AFBC` 在 HP 歸零時設、bit7 更嚴重。手冊列的人物資料畫面有 21 欄，還缺 SL、經驗、黃金、寶石、盜行、第二技能、裝備、背包。運氣（第七項屬性）的位置也未定 —— 屬性區與其第二份都只有六項 |
