...
| 怪物記錄 `+17 >> 5` 是「每次輪到時真的行動的機率」，而且索引會走到 14、一半落在 `ds:4DC0` 表外 | **是抗魔法百分比**，索引在原版是 `(b17 >> 5) & 7`，**`& 7` 保證落在八項之內**。用法在 `sub_1714A`：非 0 時擲 `rand(施法者等級, 90)`，抗性大於擲值就擋下整個法術 | 追 `sub_1714A` 的抗性判定時追到 `ds:9E31`，再回頭找它的寫入處（`2COMBAT.img` `0x13E1E`）。表的值 `[0,10,20,35,50,75,90,100]` 是一道乾淨的百分比階梯，而擲值上限 90 讓最高階正好是「免疫」—— 當成行動機率解釋不出這個上限 |
