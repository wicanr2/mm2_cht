...
| **事件腳本的 opcode** | 50 個 handler 位址與長度已全部列出，語意解出二十八個：三個顯示字串（`01`/`02`/`04`）、兩對條件跳躍（`2b` 與 `10`/`11`）、兩個固定遭遇（`12`/`13`）、事件消耗（`14`）、重畫（`0f`），再加上**讀寫角色欄位的 `15`／`18`**（經 `sub_1AA00` 的 128 項選擇器跳表換成記錄偏移）、**隊伍持有物品的 `16`**、**等按鍵的 `07`**、**換地圖的 `0c`**（地圖 = 參數 & 0x3F、座標低 nibble 是 X 高 nibble 是 Y；57 種寫死的目標全部落在 60 張圖內）、擲骰 `1c`、門檻檢查 `1b`、重畫 `29`，以及**欄位加減的 `1f`／`20`**（付款就是 `20`：扣不動時不寫回並把結果清成 0，而 `10`／`11` 讀的正是那個位元組）、**問 Y／N 的 `09`／`0a`**、**播放曲子的 `0d`**。再加上**另外三個顯示字串的 `03`／`05`／`06`**（一共六個）、**進城鎮設施的 `0e`**（1 旅店、2 訓練基地、3 酒館、4 神殿、5 法師公會、6 鐵匠 —— 五座城鎮各一個，一個不多一個不少）、**數第二技能的 `32`**（`sub_36A6`，與野外通行判定同一支）。按出現次數算，前十名全部解掉；**會顯示訊息的事件格從 57.0% 升到 63.0%**。見 [`docs/formats/07`](docs/formats/07-event-script.md) |
