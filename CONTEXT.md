...
| **事件腳本的 opcode** | 50 個 handler 位址與長度已全部列出，語意解出十三個：三個顯示字串（`01`/`02`/`04`）、兩對條件跳躍（`2b` 與 `10`/`11`）、兩個固定遭遇（`12`/`13`）、事件消耗（`14`）、重畫（`0f`），再加上**讀寫角色欄位的 `15`／`18`**（經 `sub_1AA00` 的 128 項選擇器跳表換成記錄偏移）、**隊伍持有物品的 `16`**、**等按鍵的 `07`**。按出現次數算，前十名解掉八個。見 [`docs/formats/07`](docs/formats/07-event-script.md) |
