...
| **地圖屬性表** | `ATTRIB.DAT` 已解：64 × 60，一張地圖一筆。**`+18` 是撞門的難度門檻**（由 `2MISC.img` `0xC1E4` 定出）：五座城鎮 10–40、野外全 0、地城 20–100。`+0` 是自己的編號（六十筆 0…59 一個不差，這也釘死了 stride）。`+5`…`+8` 是四方向的鄰接地圖，兩軸的對向性都是 60/60。五座城鎮四面自指、野外正好 20 張互連，與手冊的 A1–E4 吻合。見 [`docs/formats/02`](docs/formats/02-data-files.md) §8 |
