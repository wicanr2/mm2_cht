...
| **通行規則** | 已用原版量到四個方向的第一道阻擋。東西兩向支持「屬性 0 就進不去」，**南向推翻它**（(7,11) 屬性 `0x84` 非 0 卻走不動）。方向是「牆在格子之間」，Middlegate 的屬性值只用 bit0/2/4/6+bit7，與四個方向欄位的形狀吻合，但位元與方位的對應未定。見 [`docs/formats/06`](docs/formats/06-map.md) §4 |
