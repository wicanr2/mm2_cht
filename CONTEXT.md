...
| **戰鬥判定已換成原版的** | `sub_8398` 逐行讀出：命中率 = 命中門檻[難度層] − 目標防護等級（保底 5%），每次揮擊各擲 `rand(10,1009)/10` 判定，命中的傷害是 `rand(1, 骰面數)`。門檻表 `ds:103A` 與怪物的生命／經驗／攻擊次數／傷害骰全部到位。隊伍打怪物走的是**另一條路徑**（`sub_8E81`），也已解出：5% 直接命中、3% 直接落空，其餘擲 `rand(1, 25 + 等級/命中除數)` 加命中加成，要 ≥ 目標防護。怪物的防護等級來自記錄第 22 個位元組。**仍是假設的**：武器的骰面數與加成（欄位在 +76…+79，但由裝備算出來，裝備欄未解）、怪物的速度。見 [`docs/formats/08`](docs/formats/08-combat.md) |
| 角色記錄的 SP | `+88`/`+90`（uint16 目前／上限）。只有牧師與巫師非零，與手冊「遊俠與弓箭手要高經驗等級才有法力」一致 |
| 角色記錄的其餘欄位 | 已定位 23 個。root 的人物資料畫面繪製函式對每個欄位呼叫一次 `sub_29DE(序號, 值)`，把序號與偏移一次全列了出來；語意再用名冊四十筆判。新增：`+36` 防護等級、`+92` 寶石、`+98` 經驗值（uint32）、`+102` 黃金（uint32）、`+114` 法力等級、`+115` 運氣、`+116` 盜行。原有的：名稱、性別、陣營、種族、職業、六項屬性、等級、年齡、食物、狀況、HP、SP，以及屬性的第二份。狀況（+38）是位元遮罩，bit6 由 `sub_1AFBC` 在 HP 歸零時設、bit7 更嚴重。手冊列的人物資料畫面有 21 欄，還缺第二技能、裝備欄、背包。運氣（第七項屬性）的位置也未定 —— 屬性區與其第二份都只有六項 |
