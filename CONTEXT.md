...
| `MAP.DAT` 其餘位元 | **地形層的 `bit 6/4/2/0` 已證實：分辨那面牆是實牆還是門**。統計推論（只看有牆的面時對向一致率 91.7%）與原版程式碼互相印證 —— `sub_5E68` 取 `(屬性層位元 << 1 | 地形層位元)` 這個 2 位元值當訊息序號，3=`Solid!`、2=`Locked!`，而 `and al, 55h` 同時證實牆位元就是 (6,4,2,0)。屬性層的高位元平面（bit 7/5/3/1）仍未定 |
