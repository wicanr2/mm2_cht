...
**所有事件位置的斷言都建立在「`EVENTSI`／`EVENTSO` 的第 N 段 ＝ 地圖 N」
這個假設上，而它撐不住。**

判準（`TestSegmentIsMapIndex`）：`EVENTSI` 若是室內、`EVENTSO` 若是室外，
有內容的段就該落在對應類型的地圖上（室內＝`ATTRIB` `+18` 非 0）。實測：

| 檔 | 有內容 | 落對 | 落錯 |
|---|---|---|---|
| `EVENTSI` | 44 段 | 21 | 23（含 5–16 這批野外圖）|
| `EVENTSO` | 27 段 | 12 | 15（含 0–4、17–23 這批室內圖）|

兩邊各有一半落錯，所以那個對應不是這麼簡單。實機也對不上：
中門西門的腳本寫「傳送到地圖 11」，實際落在地圖 4。

另外兩個候選也對不上規模：

| 候選 | 需要 | 實際 |
|---|---|---|
| 段號 ＝ 室內圖的第幾張（`EVENTSI`）| 36 段 | 44 段有內容 |
| 段號 ＝ 室外圖的第幾張（`EVENTSO`）| 24 段 | 27 段有內容 |

三個都不成立，所以對應**多半來自一張間接表** —— 載入事件段的程式碼
會從地圖號算出段號，找到那段計算就有答案。那是下一個著手點。

**受影響的斷言**（在對應釐清之前都只是「某一段裡的某一格」，
不能指名是哪張地圖）：固定遭遇的座標、設施入口格、城門格。
格內偏移（X／Y）與腳本內容不受影響 —— 那些是段內的事實。
