...
| `MAP.DAT` 其餘位元 | **地形層的 `bit 6/4/2/0` 分辨實牆與門（強推論）**：只看有牆的面時對向一致率 91.7%，中門 379 面牆裡 7 面是門且成群。原版的 `sub_5E68` 證實了「每方向 2 位元、3=`Solid!`、2=`Locked!`」與牆位元 (6,4,2,0)（`and al, 55h`），但**那兩個位元各自取自哪個平面還沒定** —— 同位元組的讀法算出 92.8% 的牆是門，荒謬。屬性層的高位元平面（bit 7/5/3/1）仍未定 |
