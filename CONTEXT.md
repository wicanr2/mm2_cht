...
`EVENTSI.DAT` 的**開頭是一張 71 項的 32 位元偏移表**，用**地圖號**索引，
值是該地圖事件資料在檔內的位元組偏移，**0 表示這張圖沒有室內事件**。
表本身佔 `71 × 4 = 284` bytes，而第一個非零值正好是 284 —— 表尾接著
就是第一段資料。

44 個非零項，地圖號是 `0–4, 17–31, …` —— **正是室內圖那一批**
（`ATTRIB` `+18` 非 0）。
...
先前把「解析器掃出來的第幾段」當成地圖號，那是錯的 ——
掃描產生的是**非零項的序號**。前五段剛好等於地圖 0–4 所以看不出來，
第六段其實是**地圖 17**，不是地圖 5。這就是實機落點對不上的原因。

`EVENTSO.DAT` **是同一種佈局**：一樣 71 項、27 個非零，
地圖號 `5–16, 33–44, 64, 65, 68` —— 正是室外圖那一批。
它的第 0 項是 0，因為地圖 0（中門）是室內圖、沒有室外事件。

> 一度以為 `EVENTSO` 格式不同，是因為拿第 0 項去推表長（`值 ÷ 4`）——
> 那個推法只在第 0 項非零時成立。**表長是固定的 71，不是算出來的。**

兩個檔的地圖號集合互不重疊、聯集 71 個，與 `ATTRIB` 的室內／室外
分類逐張吻合（`TestEventOffsetTable`）。
