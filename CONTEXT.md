...
| `MONSTERS.16` RLE | 段內索引、動畫序列表、影像頭 x/y/w/h 已解，像素編碼未解 —— **但已確認解碼在顯示驅動裡**（走 `ds:0x4A24` 的 far pointer），不在 MM2.EXE 或 overlay 裡；功能碼對到驅動跳表第幾項還沒證實，六種用猜的讀法都已排除（見 docs/formats/04）。**哪隻怪物用哪一段已解**：怪物記錄 +0x15 |
...
| **戰鬥判定已換成原版的** | `sub_8398` 逐行讀出：命中率 = 命中門檻[難度層] − 目標防護等級（保底 5%），每次揮擊各擲 `rand(10,1009)/10` 判定，命中的傷害是 `rand(1, 骰面數)`。門檻表 `ds:103A` 與怪物的生命／經驗／攻擊次數／傷害骰全部到位。隊伍打怪物走的是**另一條路徑**（`sub_8E81`），也已解出：5% 直接命中、3% 直接落空，其餘擲 `rand(1, 25 + 等級/命中除數)` 加命中加成，要 ≥ 目標防護。怪物的防護等級來自記錄第 22 個位元組。武器的骰面數改由**已裝備的物品**算出來（`sub_CE12` 的規則，物品表 +0x10 是骰面數）。怪物的影像段號在記錄 +0x15（`b & 0x7F`，範圍 1–60，對上 `MONSTERS.16` 的 59 個段）。**仍是假設的**：怪物的速度（還沒指認出欄位）、護甲類加防護等級的累加位置。見 [`docs/formats/08`](docs/formats/08-combat.md) |
