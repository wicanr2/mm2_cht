...
| **事件腳本的 opcode** | 50 個 handler 位址與長度已全部列出，語意解出三十一個：三個顯示字串（`01`/`02`/`04`）、兩對條件跳躍（`2b` 與 `10`/`11`）、兩個固定遭遇（`12`/`13`）、事件消耗（`14`）、重畫（`0f`），再加上**讀寫角色欄位的 `15`／`18`**（經 `sub_1AA00` 的 128 項選擇器跳表換成記錄偏移）、**隊伍持有物品的 `16`**、**等按鍵的 `07`**、**換地圖的 `0c`**（地圖 = 參數 & 0x3F、座標低 nibble 是 X 高 nibble 是 Y；57 種寫死的目標全部落在 60 張圖內）、擲骰 `1c`、門檻檢查 `1b`、重畫 `29`，以及**欄位加減的 `1f`／`20`**（付款就是 `20`：扣不動時不寫回並把結果清成 0，而 `10`／`11` 讀的正是那個位元組）、**問 Y／N 的 `09`／`0a`**、**播放曲子的 `0d`**。再加上**另外三個顯示字串的 `03`／`05`／`06`**（一共六個）、**進城鎮設施的 `0e`**（1 旅店、2 訓練基地、3 酒館、4 神殿、5 法師公會、6 鐵匠 —— 五座城鎮各一個，一個不多一個不少）、**數第二技能的 `32`**（`sub_36A6`，與野外通行判定同一支）。按出現次數算，前十名全部解掉；**會顯示訊息的事件格從 57.0% 升到 63.0%**。再加上**全域變數的 `17`／`1a`**（選擇器 `0x00`–`0x17` 是 `ds:03F6` 起連續 24 個位元組的劇情旗標，腳本用到 30 種選擇器全部對得到位址）與**世紀範圍檢查的 `22`**（`ds:03CA`，43 處的參數值域 5–9，正是 MM2 時間旅行的世紀範圍）。見 [`docs/formats/07`](docs/formats/07-event-script.md) |
...
| **戰鬥判定已換成原版的** | `sub_8398` 逐行讀出：命中率 = 命中門檻[難度層] − 目標防護等級（保底 5%），每次揮擊各擲 `rand(10,1009)/10` 判定，命中的傷害是 `rand(1, 骰面數)`。門檻表 `ds:103A` 與怪物的生命／經驗／攻擊次數／傷害骰全部到位。隊伍打怪物走的是**另一條路徑**（`sub_8E81`），也已解出：5% 直接命中、3% 直接落空，其餘擲 `rand(1, 25 + 等級/命中除數)` 加命中加成，要 ≥ 目標防護。怪物的防護等級來自記錄第 22 個位元組。武器的骰面數改由**已裝備的物品**算出來（`sub_CE12` 的規則，物品表 +0x10 是骰面數）。怪物的圖號在記錄 +0x15（`b & 0x7F`，範圍 1–60）；圖號 − 1 是 `MONSTERS.16` 索引表的項次，指到空槽就往後借（`sub_6818`）。**怪物的「速度」找到了一半**：記錄 `+20` 的高 nibble 加一是**每輪最多行動幾次**（原版抄進 `ds:9F9E`，行動一次減一，用完不再輪到），已實作；同一支函式還有一道 `rand(1,100)` 對 `ds:9E25` 的擲骰，索引會走到表外，語意未定，先不實作。**仍是假設的**：護甲類加防護等級的累加位置。見 [`docs/formats/08`](docs/formats/08-combat.md) |
...
| **城鎮設施接上迴圈** | 踩到設施格就進去：旅店休息、神殿治療、訓練基地升級。**判準已換成原版的 opcode `0x0e` 子命令**（1 旅店、2 訓練基地、3 酒館、4 神殿、5 法師公會、6 鐵匠），招牌字串降為後備。子命令 7（`Brain Detoxification`）與 8 以上還沒解 |
